...
of a single class. It can be created via `matrix()` by
defining a number of rows `nrow` and columns
`ncol` of a given vector:
...
The colon in expression `1:6` in line 1 is the
...
least three dimensions (`help(array)`).
...
`[]` to address rows and columns. Both numbers must
always be separated by a comma `[line, column]`. If we
...
matrices necessarily need the same dimensionality `dim()`.
