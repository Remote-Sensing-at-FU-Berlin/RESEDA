...
The complete code is shown below. The shapefile *reg_train_data.shp* and the imagery *LC081930232017060201T1-SC20180613160412_subset.tif* can be downloaded [here](https://blogs.fu-berlin.de/reseda/prepare-samples-for-regression/##){rel="noopener" target="_blank"}.\
...
Plot your data to make sure everything is imported properly (check [Visualize in R](https://blogs.fu-berlin.de/reseda/visualize-in-r/) for an intro to plotting):
...
Time to train a Support Vector Machine [in the next section](https://blogs.fu-berlin.de/reseda/svm-regression/)!
...
Below you can see a complete code implementation. Read the [previous section](https://blogs.fu-berlin.de/reseda/prepare-samples-for-regression/){rel="noopener" target="_blank"} to learn about pre-processing the training data. While this is already executable with the given input data, you should read the in-depth guide in the following to understand the code in detail.
...
We have to set the `type` of the SVM to `"eps-regression"` in line 4 in order to perform a regression task. Furthermore, we set the kernel used in training and predicting to a [RBF kernel](https://blogs.fu-berlin.de/reseda/support-vector-machine/){rel="noopener" target="_blank"} via `"radial"` (have a look at the [SVM section](https://blogs.fu-berlin.de/reseda/support-vector-machine/){rel="noopener" target="_blank"} for more details). We set the argument `scale` to `TRUE` in order to initiate the [z-transformation of our data](https://blogs.fu-berlin.de/reseda/support-vector-machine/){rel="noopener" target="_blank"}. The argument `ranges` in line 7 takes a named list of parameter vectors spanning the sampling range. We put our vectors `gammas`, `costs` and `epsilons` in this list. By using the `tunecontrol` argument in line 8, you can set k for [the k-fold cross validation on the training data](https://blogs.fu-berlin.de/reseda/support-vector-machine/){rel="noopener" target="_blank"}, which is necessary to assess the model performance.
