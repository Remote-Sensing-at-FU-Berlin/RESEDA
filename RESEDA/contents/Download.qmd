...
You will get to know an online platform that lets you search and download Landsat 8 data -- the USGS EarthExplorer. You need dozens of images? -- No problem! Have a look at the [section for bulk and BIG DATA downloading](./Download.qmd#Landsat-BD-download) and do not forget to test your knowledge at the end of this section!
...
Landsat 8 Level-2 data is also available on demand under *Landsat* \> *Landsat Collection 1 Level-2 (On-Demand)* \> *Landsat 8 OLI/TIRS C1 Level-2* (see chapter [Landsat 8](./SensorsBasics.qmd#landsat-8){target="_blank"} for more information).
...
In order to download a single Level-1 scene (see [chapter Landsat 8](./SensorsBasics.qmd#landsat-8){target="_blank"}), click on **Download Options** ![](./img/USGS_027.png) and choose the last item, which should be the largest file (approximately 700-900 MB for Landsat 8):
...
The procedures shown in the [previous section](./Download.qmd#EarthExplorer) are sufficient if you want to acquire a few scenes. However, if you need many more scenes, e.g. if you want to do a time series analysis, the manual download becomes a real test of patience.\
...
## Bulk Download L8 Level-1 data {#Bulk}

USGS provides an application for bulk-downloading Level-1 Landsat data with a very appropriate name: the Bulk Download Application. The Bulk Download Application is an easy-to-use tool for downloading large quantities of satellite imagery and geospatial data. In order to use it, you have to order your Landsat 8 scenes via the **Add to Bulk Download** option ![](./img/usgs_039.png){.alignnone .size-full .wp-image-1314 width="18" height="18"} as mentioned in the [Download section](./Download.qmd):
...
Proceed to checkout and confirm your order as shown in the [Download section](./Download.qmd#EarthExplorer). After that, a specific order number is given to you by email and on the USGS website:
...
First of all, you should follow the guide for ordering Level-2 scenes ([Download section](./Download.qmd#EarthExplorer)) and receive the second email with the download confirmation.\
...
## BIG DATA Download {#Landsat-BD-download}
...
If you have selected the Level-1 product, select **Bulk Download** on the next page and click on **Submit Order**. Please follow the instructions given in [the bulk download section above](./Download.qmd#Bulk) to complete your download with the bulk download application.
...
If you have selected the Level-2 product, select **Order** on the next page and click on **Submit Order**, wait for the ESPA confirmation email ([Download section](./Download.qmd#EarthExplorer)) and follow the instructions given in the [Bulk Download section above](./Download.qmd#Bulk):
...
<a target="_blank" href="https://remote-sensing-at-fu-berlin.github.io/RESEDA/RESEDA/contents/EarthExplorer-Exercise"><br> <button style="width:100%;text-align:center;padding: 0;background-color:#6b9e1f;color: white">
...
[<strong>Test Your KNOWLEDGE</strong>]{style="font-size: 30px"}
:::
...
::: {style="margin: -30px 0 20px 0;text-align: center"}
[Terminal with exemplary download command for 100 Sentinel 2 products]{style="color: #686868;font-size: small"}
:::
