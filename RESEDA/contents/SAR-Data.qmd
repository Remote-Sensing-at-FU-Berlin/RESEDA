...
If you need help with downloading Sentinel data, just have a look at the [previous download chapter](https://blogs.fu-berlin.de/reseda/esa-scihub/){rel="noopener" target="_blank"}. The exemplary S1 scene was acquired over Berlin and shows an urban rural gradient.
...
Got the S1 scene (or another one)? -- Then start [SNAP](https://blogs.fu-berlin.de/reseda/snap/) now!
...
When everything is set, you might want to save your graph. Simply click on "Save" on the bottom of the graph window and choose a file path. After that, start the graph via "Run". Depending on the size of the study area and the processing nodes, this will take some time. If you run into some Java memory problems here, please refer to the [troubleshooting section](https://blogs.fu-berlin.de/reseda/intensity-images/#2) below.

Have a look at the [Bulk Processing Section](https://blogs.fu-berlin.de/reseda/s2-bulk-preprocessing/#1){rel="noopener" target="_blank"} of this course to learn, how to automate the process for many datasets!
...
Luckily, the derivation of GLCM is done in one command in SNAP. Nevertheless, we want to create a graph file that theoretically allows you to [process a lot of data easily via GPT](https://blogs.fu-berlin.de/reseda/s2-bulk-preprocessing/#1){rel="noopener" target="_blank"}. Have a look at the [Bulk Processing Section](https://blogs.fu-berlin.de/reseda/s2-bulk-preprocessing/#1){rel="noopener" target="_blank"} of this course to learn, how to automate the process for many datasets.\
...
First, get a pre-processed S1 image or another raster file of your study area. In the following example, we will use the VH-intensity image of a Sentinel 1 scene, which we derived in [the previous section](https://blogs.fu-berlin.de/reseda/intensity-images/){rel="noopener" target="_blank"}. You can download this example dataset [here](https://blogs.fu-berlin.de/reseda/texture-metrics-glcm/##){rel="noopener" target="_blank"}. It is not advantageous to use a speckle filter before deriving textures, as this eliminates most of the scene's characteristics.
